...
class Album(models.Model):
    ...
    author                        = models.ForeignKey(User, on_delete= models.CASCADE,related_name='album_authors')
    summary                       = models.CharField(max_length=2000, blank=True, default='')
    picture                       = models.ImageField(upload_to='albums/', null=True, blank=True, height_field='picture_height', width_field='picture_width')
    ...
    follows                       = models.ManyToManyField(User, blank=True, related_name='album_follows')
    ...
    def _generate_slug(self):
        ...
        for i in itertools.count(1):
            if not Album.objects.filter(slug=slug_candidate).exists():
                break
